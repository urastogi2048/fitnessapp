...
final weeklyDaywiseProvider = FutureProvider.autoDispose<WeeklyDaywise>((
  ref,
) async {
...
final weeklyBodyPartwiseProvider =
    FutureProvider.autoDispose<WeeklyBodyPartwise>((ref) async {
      final service = ref.read(statsServiceProvider);
      final token = await TokenStorage.getToken();
      if (token == null) throw Exception('No token');
...
final monthlyDaywiseProvider = FutureProvider.autoDispose<MonthlyDaywise>((
  ref,
) async {
  final service = ref.read(statsServiceProvider);
  final token = await TokenStorage.getToken();
...
final monthlyBodyPartwiseProvider =
    FutureProvider.autoDispose<MonthlyBodyPartwise>((ref) async {
      final service = ref.read(statsServiceProvider);
      final token = await TokenStorage.getToken();
      if (token == null) throw Exception('No token');
